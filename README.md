...
the same data
Setup steps
Make the repo private. Settings > General > Danger Zone > Change
...
Push these files to your repo and turn on GitHub Pages
(Settings > Pages > Deploy from branch > main).
Open the site. The first screen asks for the token, paste it in.
It is saved in that browser's local storage only, never committed.
Sign up with a username and password for yourself, and again
